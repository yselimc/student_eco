...
- **Day:** 6 of 6 — Day 5 complete; Day 6 in progress (Docker setup landed + verified end-to-end, env.example refreshed, README rewritten docker-first, error response shape standardized + all user-facing error messages translated to Turkish)
- **Active branch:** `main`
- **Last commit:** `d988d06` feat(errors): translate service error messages to Turkish
- **Next step:** Day 6 — Polish + Demo continues. Remaining items: docs reconciliation (`docs/03-database-schema.md` + `docs/04-api-spec.md` — events drift from Day 4, profile/avatar contract from Phase 3, and the new `{detail, errors[]}` error response shape + UPPERCASE error codes from the errors work), seed-data sanity, end-to-end smoke walkthrough.
...
- **Docker setup** ✅ landed direct on main as `dfd0891` (single commit, no PR per CLAUDE.md "trivial chores can land directly on main"):
  ...
  - **Boot verified end-to-end** by user: all three services healthy, migrations ran, frontend at `:3000` and backend at `:8000/docs` reachable, login/register flows worked, uploads survived `docker compose restart`.
- **`.env.example` refresh** ✅ landed as `b7d30f9` (single commit, direct on main):
  - Rewrote `backend/.env.example` with one-line per-variable comments, `JWT_SECRET` generation hint (`openssl rand -hex 32`), and two commented `DATABASE_URL` examples (Docker Compose `db` hostname vs native dev `localhost`).
  - Added `frontend/.env.example` mirroring `frontend/.env.local` with the build-time-vs-runtime caveat for `NEXT_PUBLIC_*`.
  - Removed `frontend/.env.local.example` (duplicate of the new file).
  - Verified gitignore: `backend/.env` + `frontend/.env.local` ignored; both `.env.example` files tracked.
- **README rewrite** ✅ landed as `08a199a` (single commit, direct on main):
  - Replaced the 22-line stub with a docker-first README: tech stack with versions, `docker compose up` quick-start (the verified boot path), manual setup notes, project structure, common tasks, env-var pointers, and troubleshooting for three predictable footguns (`:5432` host-port conflict, `DATABASE_URL` commented out, `NEXT_PUBLIC_*` not rebuilt when changed). Test runner honestly flagged as not wired up rather than papered over.
- **Validation error response shape standardized + Turkish messages** ✅ landed as `0aad647` (single commit, direct on main):
  - Backend 422 handler rewritten in `backend/app/main.py` to return a stable `{detail, errors[]}` shape. Centralized translator maps Pydantic v2 error types (`string_too_short`, `too_short`, `literal_error`, `missing`, email coercion) to Turkish using a field-name → label table, so `EmailStr` / `Field`-constraint failures emit user-readable Turkish without per-field validator boilerplate. Existing `@field_validator` `ValueError`s in `schemas/{auth,buddies,events,messages,users}` translated to Turkish so they pass through the `value_error` branch verbatim.
  - `AppError` subclasses gained Turkish default messages and **UPPERCASE** error codes: `NOT_FOUND`, `FORBIDDEN`, `CONFLICT`, `VALIDATION_ERROR`, `UNAUTHENTICATED`, `BUDDY_CONTACT_REQUIRED`, `EVENT_FULL`, `UNSUPPORTED_MEDIA_TYPE`, `PAYLOAD_TOO_LARGE`. Starlette HTTPException code map follows the same casing.
  - Frontend `ApiError` (in `frontend/src/lib/api.ts`) gained `fieldErrors: Record<string, string>` populated from the new 422 shape; existing `message`/`code`/`status` untouched. `buddies/me/page.tsx` updated for the `BUDDY_CONTACT_REQUIRED` rename (only frontend consumer that string-matches an error code).
- **Service error messages translated to Turkish** ✅ landed as `d988d06` (single commit, direct on main):
  - 62 user-facing message strings across 8 service + storage files (`notes`, `buddies`, `listings`, `users`, `events`, `auth`, `messages`, `uploaders`) translated from English to Turkish. Uses resource-specific names ("Not bulunamadı", "İlan bulunamadı", "Etkinlik bulunamadı", "Kullanıcı bulunamadı", "Mesaj bulunamadı") rather than the generic "Kayıt bulunamadı" default, so the message identifies the missing entity. Same for forbidden / conflict / validation messages. No business logic changes — only message strings passed to existing raise sites. Routes untouched per architecture layer rules (routes don't raise directly).
- Remaining Day 6 work (not started): docs reconciliation (`docs/03-database-schema.md` + `docs/04-api-spec.md` drift from Day 4 events, Phase 3 profile/avatar, and the new error response shape — see Open Tech Debt), seed-data sanity, end-to-end smoke walkthrough across all 4 modules + home + profile.
...
- **Docs drift from current implementation** [Day 4 events + Phase 3 profile/avatar + Day 6 errors] — `docs/03-database-schema.md` §2.1 users is missing `university`/`department`/`avatar_path`/`updated_at`; §2.5 events lists `career` not `culture`, missing `max_attendees`, missing CHECK constraints. `docs/04-api-spec.md` §3 still describes the old `/users/me` shape (no profile DTO with counts); §6 lists `PATCH /events/{id}` that doesn't exist, missing `?mine=1` filter, wrong attendee shape (`{user_id, full_name, department}` vs actual `{user_id, display_name, avatar_url, rsvp_at}`), missing `max_attendees`; auth section missing `PATCH /auth/me` + `POST/DELETE /auth/me/avatar`; `avatar_url` field missing on several DTOs. Error response section needs the new 422 `{detail, errors[]}` shape and the UPPERCASE error-code set from `0aad647` (was previously lowercase or undocumented). Reconcile in Day 6 docs pass.
...
### 2026-05-12 — Day 6 kickoff: dockerize stack + .env.example refresh

- **Done this session:**
  - **Docker setup** — landed as `dfd0891` (direct on main, single commit, no PR per CLAUDE.md "trivial chores"). Six files: `backend/Dockerfile` (python:3.12-slim, non-root `appuser`, `pip install .`, runs `alembic upgrade head && uvicorn` on start), `frontend/Dockerfile` (multi-stage node:20-alpine → Next standalone runtime as the `node` user, `NEXT_PUBLIC_API_BASE_URL` accepted as build-arg), `frontend/next.config.mjs` (added `output: 'standalone'` — only application-adjacent change), `docker-compose.yml` (replaces the orphaned alternate-Postgres-on-:5433 file from Day 2; three services with `pg_isready`-gated startup, named volumes for `pgdata` + `uploads`, host ports 5432/8000/3000, in-container `DATABASE_URL` overridden to `db:5432` via `environment:` so `backend/.env` can keep its native-dev value), `backend/.dockerignore` + `frontend/.dockerignore` (per-spec exclude list + standard cruft). Decision #35 captures the three Docker sub-decisions (named volume vs bind mount, build-arg for `NEXT_PUBLIC_*`, env override for `DATABASE_URL`).
  - **Boot verified end-to-end by user**: all three services came up healthy, migrations ran, frontend at `:3000` worked, backend at `:8000/docs` worked, login/register flows worked, uploads persisted across `docker compose restart`.
  - **`.env.example` refresh** — landed as `b7d30f9` (direct on main, single commit). Rewrote `backend/.env.example` with one-line per-variable comments, `JWT_SECRET` generation hint, and two commented `DATABASE_URL` examples (Docker `db` hostname vs native `localhost`). Added `frontend/.env.example` mirroring `frontend/.env.local` with build-time-vs-runtime caveat for `NEXT_PUBLIC_*`. Deleted stale duplicate `frontend/.env.local.example`. Gitignore behavior verified via `git check-ignore -v`: `.env` + `.env.local` ignored; both `.env.example` files tracked.
- **Next:** Continue Day 6 — docs reconciliation (`docs/03-database-schema.md` §2.1 users + §2.5 events; `docs/04-api-spec.md` §3 user shape + §6 events drift + missing avatar endpoints — full list in Open Tech Debt), seed-data sanity, end-to-end smoke walkthrough across all 4 modules + home + profile.
- **Unresolved:**
  - Host Postgres on `:5432` will conflict with the `db` service's `5432:5432` mapping for any future fresh boot — documented in the Services section, requires `sudo systemctl stop postgresql` before `docker compose up`.
...
### 2026-05-12 — Day 6 continued: README rewrite + error response standardization + Turkish error messages

- **Done this session:**
  - **README rewrite** — landed as `08a199a` (direct on main, single commit, no PR per CLAUDE.md "trivial chores"). Replaced the 22-line stub with a docker-first README: tech stack with versions, `docker compose up` quick-start (the verified boot path), manual setup notes, project structure, common tasks, env-var pointers, troubleshooting for three predictable footguns (`:5432` host-port conflict, commented-out `DATABASE_URL`, `NEXT_PUBLIC_*` not rebuilt when changed). Test runner honestly flagged as not wired up rather than papered over.
  - **Validation error response shape standardized + Turkish messages** — landed as `0aad647` (direct on main, single commit). Rewrote the 422 handler in `backend/app/main.py` to return a stable `{detail, errors[]}` shape with Turkish per-field messages. Centralized translator maps Pydantic v2 error types (`string_too_short`, `too_short`, `literal_error`, `missing`, email coercion) to Turkish using a field-name → label table, so `EmailStr` / `Field`-constraint failures emit user-readable Turkish without per-field validator boilerplate. Existing `@field_validator` `ValueError`s in `schemas/{auth,buddies,events,messages,users}` translated to Turkish so they pass through the `value_error` branch verbatim. `AppError` subclasses (in `core/exceptions.py`) gained Turkish default messages and **UPPERCASE** error codes: `NOT_FOUND`, `FORBIDDEN`, `CONFLICT`, `VALIDATION_ERROR`, `UNAUTHENTICATED`, `BUDDY_CONTACT_REQUIRED`, `EVENT_FULL`, `UNSUPPORTED_MEDIA_TYPE`, `PAYLOAD_TOO_LARGE`. Starlette HTTPException code map follows the same casing. Frontend `ApiError` (in `frontend/src/lib/api.ts`) gained `fieldErrors: Record<string, string>` populated from the new 422 shape; existing `message`/`code`/`status` untouched. `buddies/me/page.tsx` updated for the `BUDDY_CONTACT_REQUIRED` rename (only frontend consumer that string-matches an error code).
  - **Service error messages translated to Turkish** — landed as `d988d06` (direct on main, single commit). 62 user-facing message strings across 8 service + storage files (`notes`, `buddies`, `listings`, `users`, `events`, `auth`, `messages`, `uploaders`) translated from English to Turkish. Uses resource-specific names ("Not bulunamadı", "İlan bulunamadı", "Etkinlik bulunamadı", "Kullanıcı bulunamadı", "Mesaj bulunamadı") rather than the generic "Kayıt bulunamadı" default, so the message identifies the missing entity. Same for forbidden / conflict / validation messages. No business logic changes — only message strings passed to existing raise sites. Routes/ untouched per architecture layer rules (routes don't raise directly).
  - **Docker boot remains verified end-to-end** from the prior session (`dfd0891` + reconcile `5451109`); no re-verification done for the docs / errors work since neither changes runtime boot behavior.
- **Next:** Continue Day 6 — docs reconciliation (`docs/03-database-schema.md` §2.1 users + §2.5 events; `docs/04-api-spec.md` §3 user shape + §6 events drift + missing avatar endpoints + new `{detail, errors[]}` error response shape with UPPERCASE error codes — full list in Open Tech Debt), seed-data sanity, end-to-end smoke walkthrough across all 4 modules + home + profile.
- **Unresolved:**
  - The new `{detail, errors[]}` 422 shape and UPPERCASE error-code set are not yet reflected in `docs/04-api-spec.md` — folds into the Day 6 docs pass (now bundled into the existing Open Tech Debt docs-drift entry).
  - No browser smoke test of the new field-error rendering path was done this session — `ApiError.fieldErrors` is wired but no caller consumes it yet. Pick up during the Day 6 smoke walkthrough.
  - All prior Day 6 carry-overs unchanged: host Postgres on `:5432` conflict for fresh boots, Day 4 events + Phase 3 profile/avatar docs drift, Next.js CVEs accepted-for-v1, bcrypt 72-byte truncation, localStorage JWT exposure.
